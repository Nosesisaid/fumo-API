...
pub fn edit_involved(conn: &mut PgConnection, fumo_unique_id: i64, new_involved: Vec<String>) -> QueryResult<Fumo> {
    let mut new_involved_typed: Vec<Option<String>> = vec![];

    for element in new_involved {
        if !is_valid_involvable(&element) {
            return Err(diesel::result::Error::QueryBuilderError("Invalid involvable provided".into()))
        }
    ...
    let result = update(fumos).filter(id.eq(fumo_unique_id)).set(involved.eq(new_involved_typed)).returning(Fumo::as_returning()).get_result(conn);
